node {
  ...
  stage('SonarQube Test') {
	...
	stage('ssh command') {
      ...
      sshCommand remote: remote, sudo: true, command: 'kubectl set image deployments/coursework-2 coursework-2=hub.docker.com/repository/docker/csherr2510/coursework-2:"${env.BUILD_NUMBER}"'
    }
}
